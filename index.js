...
function renderMoviesByList(data) {
  ...
  data.forEach((item) => {
    ...
    rawHTML += `<li class="list-group-item d-flex justify-content-between">
        <i class="nav-link" data-bs-toggle="pill">${item.title}</i>
        <div>
...
function renderPaginator(amount) {
  ...
  for (let page = 1; page <= numberOfPages; page++) {
    rawHTML += `<li class="page-item"><a class="page-link nav-link" data-bs-toggle="pill" href="#" data-page="${page}">${page}</a></li>`
  }
...
searchForm.addEventListener('submit', function onSearchFormSubmitted(event) {
  ...
  selectMode(getMoviesByPage(currentPage))
})
...
modeChange.addEventListener('click', (event) => {
  if (event.target.matches('.card-mode')) {
    ...
    mode = 'list'
    renderMoviesByList(getMoviesByPage(currentPage))
  }
})

// 根據mode決定要用何種方式印出movies
function selectMode(content) {
  if (mode === 'card')
    renderMoviesByCard(content)
  else if (mode === 'list')
    renderMoviesByList(content)
}
